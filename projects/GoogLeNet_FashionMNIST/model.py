...
class Inception(nn.Module):
    def __init__(self,in_channels,c1,c2,c3,c4):
        ...
        self.ReLU = nn.ReLU()
        #路线1：1*1的卷积
        self.p1_1 = nn.Conv2d(in_channels=in_channels,out_channels=c1,kernel_size=1,padding=0,stride=1)
        #路线2：
        #1*1的卷积
    ...
    def forward(self,x):
        ...
        x2 = self.ReLU(self.p2_1(x))
        x2 = self.ReLU(self.p2_2(x2))
        x3 = self.ReLU(self.p3_1(x))
        x3 = self.ReLU(self.p3_2(x3))
        x4 = self.p4_1(x)
        x4 = self.ReLU(self.p4_2(x4))
